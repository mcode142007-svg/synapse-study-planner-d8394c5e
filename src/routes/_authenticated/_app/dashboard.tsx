  ...
  return (
    <div className="space-y-5 pb-6">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-base italic text-[#B46A72] dark:text-[#F7C8D3]">Today</p>
          <h1 className="text-5xl font-semibold text-[#2D3A47] dark:text-[#FFF7E6]">
            Dashboard
          </h1>
        </div>
        <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-indigo-600 px-4 py-3 text-base font-semibold text-white"
          >
            <CalendarDays className="h-4 w-4" />
            Timeline
          </button>

          <div className="ml-2 flex items-center gap-2 rounded-md border border-[#F7C8D3]/60 bg-white/60 px-4 py-3 text-[#B46A72] shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-[#F7C8D3]">
            <Flame className="h-4 w-4" />
            <span className="text-base font-semibold">{data?.profile?.streak ?? 0}</span>
          </div>
        </div>
      </header>
        ...
        <section className="rounded-lg border border-[#F7C8D3]/70 bg-white/70 p-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5">
          <div className="mb-3 flex items-start justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-2xl font-semibold text-[#2D3A47] dark:text-[#FFF7E6]">
                AI Check-In
              </h2>
            </div>
            <button
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            {[
              ...
              <button
                key={question}
                type="button"
                className="min-h-12 rounded-md border border-[#F7C8D3]/60 bg-[#FFF7E6]/80 px-4 py-3 text-left text-base font-medium text-[#2D3A47] transition active:scale-[0.98] dark:border-white/10 dark:bg-white/5 dark:text-[#FFF7E6]"
              >
                {question}
              </button>
            ))}
        ...
        <div className="-mx-4 grid gap-3 px-4 pb-1 md:grid-cols-3">
          {readiness.length ? (
            readiness.map((goal) => (
              ...
              >
                <p className="line-clamp-1 text-base font-semibold text-[#2D3A47] dark:text-[#FFF7E6]">
                  {goal.goal_name ?? "Exam"}
                </p>
                <p className="mt-2 text-3xl font-semibold text-[#B46A72]">
                  {goal.days == null ? "—" : Math.max(goal.days, 0)}
                </p>
                <p className="text-sm uppercase tracking-wide text-[#6B7280]">days remaining</p>
      ...
      <section className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          {dashboardQuery.isLoading ? (
            <div className="rounded-lg border border-[#F7C8D3]/60 bg-white/60 p-4 text-base text-[#4B5563]">
              Loading your plan...
            </div>
          ) : data?.todaysTasks.length ? (
            data.todaysTasks.map((task) => <TaskCard key={task.id} task={task} onOpen={openTask} />)
          ) : (
            <div className="rounded-lg border border-dashed border-[#F7C8D3]/70 p-4 text-base text-[#4B5563]">
  ...
  return (
    ...
    >
      <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm uppercase tracking-wide text-[#6B7280]">
            {task.scheduled_start_time?.slice(0, 5) ?? "Anytime"}
            {task.scheduled_end_time ? `-${task.scheduled_end_time.slice(0, 5)}` : ""}
          </p>
          <h3 className="mt-1 text-2xl font-semibold leading-tight text-[#2D3A47] dark:text-[#FFF7E6]">
      ...
      <p className="mt-1 text-sm text-[#2D3A47]/70 dark:text-[#FFF7E6]/70">
        ...
        <p className="mt-3 text-sm uppercase tracking-wide text-[#B46A72]">
